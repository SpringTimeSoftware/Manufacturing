...
Runtime reliability, role seeds, smoke, role-wise UAT, live/demo data truth.

## Scope keywords

runtime, API probe, seed repair, role identities, smoke tests, UAT.
...
- Verify all role logins and menu access.
- Verify no role-critical endpoint returns HTTP 500.
- Verify live mode does not show fake seeded operational rows.
- Create realistic seed data for order-to-cash and plan-to-produce flows.
- Re-run smoke and role-wise UAT with PASS/PARTIAL/FAIL/NOT-IN-SCOPE statuses.
- Verify database apply order and publish/run path.
...
- create/update the workstream matrix under `docs/workstream-progress/WS01/`.
...
Capture screenshots under `docs/codex-review-screens/WS01/`.
If a screen scrolls, capture max top/middle/bottom.

### Phase F — Final output
Create:
- `docs/codex-progress/WS01-output.md`

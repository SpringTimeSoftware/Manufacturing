...
User/role/security/admin/UDF/audit foundation.

## Scope keywords

users, roles, permissions, audit, approvals, workflow, numbering, tenant settings, language, UDF, attachments, rate limiting.
...
- Complete audit viewer API/UI and audit filtering.
- Complete users, roles, permissions, workflow, numbering, language, tenant settings.
- Add rate limiting for login, AI, import/export, and integrations.
- Add attachment authorization tests for preview/download.
- Add UDF foundation: field definitions, entity mapping, storage, validation, audit, role visibility.
- Ensure approvals have matrix, escalation, history, and linked-record truth.
...
- create/update the workstream matrix under `docs/workstream-progress/WS02/`.
...
Capture screenshots under `docs/codex-review-screens/WS02/`.
If a screen scrolls, capture max top/middle/bottom.

### Phase F — Final output
Create:
- `docs/codex-progress/WS02-output.md`
